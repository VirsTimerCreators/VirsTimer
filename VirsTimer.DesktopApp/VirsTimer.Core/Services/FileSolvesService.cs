...
namespace VirsTimer.Core.Services
{
    public partial class FileSolvesService : IPastSolvesGetter, ISolvesSaver, IEventsGetter, ISessionsManager
    {
        ...
        public Task<IReadOnlyList<Session>> GetSessionsAsync(string @event)
        {
            var targetDirectory = _fileSystem.Path.Combine(Application.ApplicationDataDirectoryPath, @event);
        ...
        public Task AddSessionAsync(string @event, Session session)
        {
            var targetFile = _fileSystem.Path.Combine(Application.ApplicationDataDirectoryPath, @event, $"{session.Name}{FileExtensions.Json}");
            _fileSystem.Directory.CreateDirectory(_fileSystem.Path.GetDirectoryName(targetFile));
            if (!_fileSystem.File.Exists(targetFile))
